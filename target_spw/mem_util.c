...
#include "jasmine.h"
static BOOL32 is_backupzone_block(UINT32 const bank, UINT32 const vblk_offset);
...
static BOOL32 is_backupzone_block(UINT32 const bank, UINT32 const vblk_offset)
{
    ...
    if (tst_bit_dram(BACKUP_ZONE_BMP_ADDR + bank*(VBLKS_PER_BANK/8 + 1), vblk_offset) == FALSE)
    {
        return FALSE;
    }
    return TRUE;
...
UINT32 _mem_search_min_max(const void* const addr, UINT32 const num_bytes_per_item, UINT32 const num_items, UINT32 const cmd,UINT32 const bank)//bank도 받게함
{
	UINT32 retval;
	UINT32 vcount;
	...
	{
		retval = GETREG(MU_RESULT);
		//backupzone block이면 그냥 넘긴다. victim block으로 삼지 않음.
		
		if(is_backupzone_block(bank,retval%VBLKS_PER_BANK)==TRUE)
			continue;
		
		
		//uart_print("1111111111111111111111111");
		vcount = read_dram_16(VCOUNT_ADDR + (((bank * VBLKS_PER_BANK) + retval) * sizeof(UINT16)));
		
		if(vcount==0xCECE)
			continue;
		else if (retval != 0xFFFFFFFF)
			break;
	}
