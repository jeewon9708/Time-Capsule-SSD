...
+ BAD_BLK_BMP_BYTES + PAGE_MAP_BYTES + VCOUNT_BYTES + FLUSH_MAP_BYTES+RECLAIMABILITY_BMP_BYTES+BACKUP_ZONE_BMP_BYTES+BACKUP_ZONE_LPN_BYTES)//+METADATA_BACKUPZONE_BYTES)  //flush
...
#define RECLAIMABILITY_BMP_BYTES (((NUM_VPAGES / 8) + 1 ) + (DRAM_ECC_UNIT - 1)) / DRAM_ECC_UNIT * DRAM_ECC_UNIT //DRAM_ECC_UNIT 의 배수여야함
...
//backup zone meta data for each active block

#define BACKUP_BLOCK_MAX_NUM 10
/*
#define METADATA_BACKUPZONE_ADDR (BACKUP_ZONE_LPN_MAP + BACKUP_ZONE_LPN_BYTES)
#define METADATA_BACKUPZONE_BYTES 128*8*BACKUP_BLOCK_MAX_NUM
*/
