...
static UINT32 queue_pop(UINT32* lba, UINT32* sector_count, UINT32* cmd_type,UINT32 *ret_time)
{
	if(!queue_isEmpty())
	{
		...
		*ret_time=eve_q[eveq_front].retention_duration;
...
static void eventq_get(CMD_T* cmd)
{
	...
	queue_pop( &(cmd->lba), &(cmd->sector_count), &(cmd->cmd_type),&(cmd->retention_duration));
...
void Main(void)
{
	...
	while (1)
	{
		if (eventq_get_count())
		{
			...
			{
				//write할 때마다 이니깐 온 값 받아서 write할 때 같이 보내주자
				ftl_write(cmd.lba, cmd.sector_count,cmd.retention_duration);
				//ftl_write(cmd.lba, cmd.sector_count,60);
