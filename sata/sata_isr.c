...
static __inline void queue_push(UINT32 lba, UINT32 sector_count, UINT32 cmd_type,UINT32 retention_duration)
{
	if(eveq_init==1)
	{
		eveq_front=0;
	...
	if(!queue_isFull())
	{
		...
		eve_q[eveq_rear].retention_duration = retention_duration;

		eveq_rear = (eveq_rear+1) % Q_SIZE;
		SETREG(SATA_EQ_STATUS, ((eveq_rear-eveq_front) & 0xFF)<<16);
	//	uart_printf("queue push : lba : %d, sec_cnt: %d, type : %d, retiontion_duration : %d",lba,sector_count,cmd_type,retention_duration);
	}
}
...
static __inline void handle_got_cfis(void)
{
	...
	RET_duration = GETREG(SATA_FIS_H2D_4) & 0xFFFFFFFF;
	...
	else if (cmd_type & (CCL_FTL_H2D | CCL_FTL_D2H))
	{
		...
		{//write command일 경우
			...
			queue_push(lba, sector_count, 1, RET_duration);	//key
		...
		else
		{
			...
			queue_push(lba, sector_count, 0,0);	//key
